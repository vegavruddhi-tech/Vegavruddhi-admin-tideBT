/**
 * Sync BT_TL_CONNECT JAN 26 from sheet 1ebvpDwSwPV6s-C2QPAkRserFv2yV_6n1NujDZZj4ako
 * Tab: BT TL CONNECT JAN 26  →  Collection: BT_TL_CONNECT JAN 26
 */
...
async function run() {
  ...
  const tabName         = 'BT TL CONNECT JAN 26';
  const mongoCollection = 'BT_TL_CONNECT JAN 26';
  ...
  rows.slice(1).forEach((row, i) => {
    ...
    docs.push({
      month:'January', partnerName: getVal(row,iPartner), teamLeadName: getVal(row,iTL), lead: getVal(row,iLead),
