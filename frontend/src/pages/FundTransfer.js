...
export default function FundTransfer() {
  ...
  const fetchData = async () => {
    try {
      const [fseRes, tlRes, paymentsRes] = await Promise.all([
  ...
  }, [payments, dateFilter, fromDate, toDate, selectedYear, selectedMonth]);
          ...
          <Card sx={{ borderRadius: 2, border: '1.5px solid #e0e0e0' }}>
            <CardContent>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
                ...
                <Chip label={`₹${totalAmount.toLocaleString()}`} sx={{ bgcolor: '#e6f4ea', color: '#2e7d32', fontWeight: 700 }} />
              </Box>
